<?php
	...
	*/
	...
	$edit_comments = strip_tags($_POST['edit_comments']);
	
	if ($ovAdminSecurity->IsAdminLoggedIn() && $ovAdminSecurity->CanAccessPreferences())
	{
		$ovAdminSettings->SaveCommentSettings($edit_comments);

		header("Location: /ov-admin/settings?page=comments");
		exit();
	}
	...
	{
		header("Location: /ov-admin/login?redirecturl=" . urlencode("/ov-admin/settings?page=comments"));
		exit();
	}
?>
<?php
	...
	*/
	...
	switch ($login_result)
	{
		...
		case "SUSPENDED":
			header("LOCATION: /login?error=2&redirecturl=" . urlencode($previous_page));
			exit();
			break;
		case "BANNED":
			header("LOCATION: /login?error=3&redirecturl=" . urlencode($previous_page));
			exit();
			break;
		case "IP BANNED":
			header("LOCATION: /login?error=4&redirecturl=" . urlencode($previous_page));
			exit();
			break;
		case "INVALID":
			header("LOCATION: /login?error=1&redirecturl=" . urlencode($previous_page));
			exit();
			break;
		case "OK":
